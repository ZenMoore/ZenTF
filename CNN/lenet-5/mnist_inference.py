...
#增加参数train用于区分训练过程与测试过程，在训练过程中使用dropout方法，dropout可以进一步提升模型可靠性并防止过拟合，dropout只在训练时候使用
def inference(input_tensor, train, regularizer):
    ...
    #将池化层输出拉成一个向量。get_shape函数可以得到维度
    pool_shape = pool2.get_shape().as_list()
    nodes = pool_shape[1] * pool_shape[2] * pool_shape[3]

    reshaped = tf.reshape(pool2, [pool_shape[0], nodes])    #todo

    #dropout在训练时会随机将部分节点的输出改为0. dropout可以避免过拟合问题，一般只在全连接层而不是卷积层或者池化层使用
